...
import { projectName, sameProjectPath, sessionTitle, useWorkspace } from "../workspace/WorkspaceContext";
...
export function Dashboard(props: DashboardProps) {
  ...
  const currentPath = state.project?.path;
  const currentProject = currentPath ? workspace.projects.find(project => sameProjectPath(project.path, currentPath)) : undefined;
  const openPath = currentProject?.path;
  return <section className="page-section workspace-page dashboard-page" aria-labelledby="home-title" data-webcodex-page="home">
    <header className="page-heading-row">
      <div><span className="eyebrow">{p("workspace")}</span><h1 id="home-title">{currentProject ? projectName(currentProject) : "WebCodex"}</h1>{currentProject?.path && <code className="dashboard-project-path">{currentProject.path}</code>}</div>
      <div className="dashboard-heading-actions">
        {openPath && <Button className="primary-button" disabled={busy} onClick={() => props.onOpenProject(openPath)}>{p("openProject")}</Button>}
