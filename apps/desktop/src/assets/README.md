...
`frontend/src/ui/brand-mark.svg` is the single source for the Runtime, Desktop,
and Admin mark. `app-icon.png` is its generated 1024px desktop artwork and
`brand.png` is its generated 128px in-app version.
...
and should use a macOS run when the source artwork changes. The checked-in ICNS
for this revision was refreshed from the same SVG source. Only desktop outputs
