...
export function createWindowCard(
  ...
  const source = document.createElement("span"); source.className = "muted small";
  source.textContent = translate("Source", language) + " · " + String(row.source || "—"); button.appendChild(source);
  const seen = document.createElement("span"); seen.className = "muted small";
  seen.textContent = translate("Last seen", language) + " · " + windowAgeLabel(row.last_seen_at_ms, now, language); button.appendChild(seen);
